...
PDAC_AD_cells <- RunUMAP(PDAC_AD_cells,dims = 1:6,n.neighbors = 5,reduction = 'harmony',reduction.name = 'umap_harmony',seed.use = 42)
png('KLF5 overview harmony seed 5 ndim 25 umap ndim 6.png',width = 1250,height = 1000,units = 'px',res = 300)
FeaturePlot(PDAC_AD_cells,'KLF5')+NoAxes()
dev.off()
...
print(p1/p2)
dev.off()
plot_gene_trajectory(PDAC_AD_cells,c('KLF5'),method_to_use = 'median',pseudotime_cap = c(70,75),x_lim = c(0,80),
                     output_width = 2000,output_height = 1200, bin_size = 10,violin_width = 5)
# Partition cells into different stages in monocle3
PDAC_AD_cells_cds@colData$pseudotime_monocle3 <- pseudotime(PDAC_AD_cells_cds, reduction_method = 'UMAP')
...
PDAC_AD_cells_cds@colData$trajectory_position <- 'Other'

early_transition_cells <- intersect(colnames(PDAC_DT2_cells)[PDAC_DT2_cells$pseudotime>=28 & PDAC_DT2_cells$pseudotime<=32.5],
                                    low_SPINK1_trajectory)
PDAC_DT2_cells$trajectory_position[early_transition_cells] <- 'Early transition'
PDAC_AD_cells_cds@colData[early_transition_cells,'trajectory_position'] <- 'Early transition'

transition_cells <- intersect(colnames(PDAC_DT2_cells)[PDAC_DT2_cells$pseudotime>32.5 & PDAC_DT2_cells$pseudotime<=42],
                              low_SPINK1_trajectory)
PDAC_DT2_cells$trajectory_position[transition_cells] <- 'Transition'
PDAC_AD_cells_cds@colData[transition_cells,'trajectory_position'] <- 'Transition'
...
PDAC_AD_cells_cds@colData[bifurcation_cells,'trajectory_position'] <- 'Bifurcation'
...
PDAC_AD_cells_cds@colData[high_SPINK1_end_cells,'trajectory_position'] <- 'High SPINK1 end'
...
PDAC_AD_cells_cds@colData[low_SPINK1_end_cells,'trajectory_position'] <- 'Low SPINK1 end'


PDAC_DT2_cells$trajectory_position <- factor(PDAC_DT2_cells$trajectory_position,
                                             levels = c('Early transition','Transition','Other','Bifurcation','High SPINK1 end','Low SPINK1 end'))
PDAC_AD_cells_cds@colData$trajectory_position <- factor(PDAC_AD_cells_cds@colData$trajectory_position,
                                             levels = c('Early transition','Transition','Other','Bifurcation','High SPINK1 end','Low SPINK1 end'))
setwd('C:\\Users\\xshan\\OneDrive\\Desktop\\R code\\PDAC_reference\\final_results_v4_KLF5')
png('Trajectory overlapped with cells at different stages no legend.png',width = 1000,height = 1000,res = 300)
plot_cells(PDAC_AD_cells_cds, color_cells_by = 'trajectory_position', label_cell_groups=FALSE,
           label_leaves=FALSE, label_branch_points=FALSE,
           graph_label_size=3, trajectory_graph_color = 'blueviolet', 
           trajectory_graph_segment_size = 1.5)+NoAxes()+NoLegend()+
  scale_color_manual(values = c('palegreen3','#70992e','lightgrey','#537222','#e0c244','#298982'))
  #scale_color_manual(values = c('chartreuse2','chartreuse4','gold3','deepskyblue','grey'),
                     #labels = c('Transition','Bifurcation','High KLF5 end','Low KLF5 end','Other'))+
...
# Check expression patterns of DM negative genes
PDAC_AD_cells <- readRDS('C:\\Users\\xshan\\OneDrive\\Desktop\\R code\\PDAC_reference\\reference_dataset_core\\PDAC_acinar_ductal_cells_15430_genes_Harmony_by_patients_v4.rds')
PDAC_DT2_cells <- readRDS('C:\\Users\\xshan\\OneDrive\\Desktop\\R code\\PDAC_reference\\reference_dataset_core\\PDAC_ductal_type_2_cells_15430_genes_Harmony_by_patients_v4.rds')
setwd('C:\\Users\\xshan\\OneDrive\\Desktop\\R code\\PDAC_reference\\final_results_v4_KLF5')
png('KLF5 DT2.png',width = 1000,height = 1000,res = 300)
FeaturePlot(PDAC_DT2_cells, c('KLF5'))+NoAxes()
dev.off()
setwd('C:\\Users\\xshan\\OneDrive\\Desktop\\R code\\PDAC_reference\\final_results_v4_KLF5')
png('DM_negative genes.png',width = 2000,height = 2000,res = 300)
FeaturePlot(PDAC_DT2_cells, c('MTHFD1','NCAPD2','PLK4','RNASEH2A'),ncol = 2,order = TRUE)&NoAxes()
dev.off()
png('Stem cell markers.png',width = 2000,height = 2000,res = 300)
FeaturePlot(PDAC_DT2_cells, c('CD44','EPCAM','ALDH1A1','PROM1'),ncol = 2)&NoAxes()
dev.off()
high_vs_transition_markers <- readRDS('high_vs_transition_markers.rds')
high_vs_transition_markers[c('MTHFD1','NCAPD2','PLK4','RNASEH2A'),]
# Compare CD44
PDAC_DT2_cells$trajectory_position_highres <- PDAC_DT2_cells$trajectory_position
PDAC_DT2_cells$trajectory_position_highres <- factor(PDAC_DT2_cells$trajectory_position_highres,
                                                     levels = c('Early transition','Transition',
                                                                'Other','Bifurcation','High SPINK1 end','Low SPINK1 end'))
PDAC_DT2_cells$trajectory_position_highres[PDAC_DT2_cells$pseudotime>=28 & PDAC_DT2_cells$pseudotime<=32.5] <- 'Early transition'

png('Trajectory position map.png',width = 1000,height = 1000,res = 300)
DimPlot(PDAC_DT2_cells,group.by = 'trajectory_position_highres')+
  ggtitle(NULL)+NoAxes()+NoLegend()+
  scale_color_manual(values = c('palegreen3','#70992e','lightgrey','#537222','#e0c244','#298982'))
dev.off()

Idents(PDAC_DT2_cells) <- 'trajectory_position'
high_vs_early_transition <- FindMarkers(PDAC_DT2_cells,
                                        ident.1 = 'High SPINK1 end',ident.2 = 'Early transition')

high_vs_bifurcation <- FindMarkers(PDAC_DT2_cells,
                                   ident.1 = 'High SPINK1 end',ident.2 = 'Bifurcation')
high_vs_bifurcation[c('MTHFD1','NCAPD2','PLK4','RNASEH2A','CD44'),]

p1 <- FeaturePlot(PDAC_DT2_cells,c('ELF3'))+NoAxes()
p2 <- FeaturePlot(PDAC_DT2_cells,c('CDH1'))+NoAxes()
p3 <- FeaturePlot(PDAC_DT2_cells,c('FN1'),max.cutoff = 3,order = TRUE)+NoAxes()
png('EMT related genes.png',width = 3000,height = 1000,res = 300)
print(p1|p2|p3)
dev.off()

p1 <- FeaturePlot(PDAC_DT2_cells,c('CD44'))+NoAxes()
p2 <- FeaturePlot(PDAC_DT2_cells,c('EMP1'),order = TRUE)+NoAxes()
png('Cluster 2 and 5 markers.png',width = 1000,height = 2000,res = 300)
print(p1/p2)
dev.off()

FeaturePlot(PDAC_DT2_cells,c('TGFBR1','TGFBR2'),order = TRUE)&NoAxes()

###########DEG analysis for data from Feinberge######################
setwd('C:\\Users\\xshan\\OneDrive\\Desktop\\R code\\PDAC_reference\\final_results_v4_KLF5\\DEG analysis')
DEGs_collection <- vector(mode = 'list',length = 5)
for(i_cluster in 1:5)
{
  DEGs_collection[[i_cluster]] <- read.csv(sprintf('A13D_vs_A13B_cluster_%d.csv',i_cluster))
}
DEGs_collection_filtered <- vector(mode = 'list',length = 5)
for(i_cluster in 1:5)
{
  to_filter <- DEGs_collection[[i_cluster]]
  to_filter <- to_filter[to_filter$p_val_adj<=0.05,]
  to_filter <- to_filter[order(to_filter$avg_log2FC,decreasing = TRUE),]
  DEGs_collection_filtered[[i_cluster]] <- to_filter
}

for(i_cluster in 1:5)
{
  DEGs <- DEGs_collection_filtered[[i_cluster]]$X[DEGs_collection_filtered[[i_cluster]]$avg_log2FC>=0]
  writeLines(DEGs,sprintf('Upregulated threshold 0 cluster %d.txt',i_cluster))
  DEGs <- DEGs_collection_filtered[[i_cluster]]$X[DEGs_collection_filtered[[i_cluster]]$avg_log2FC<=0]
  writeLines(DEGs,sprintf('Downregulated threshold 0 cluster %d.txt',i_cluster))
  
  DEGs <- DEGs_collection_filtered[[i_cluster]]$X[DEGs_collection_filtered[[i_cluster]]$avg_log2FC>=1]
  writeLines(DEGs,sprintf('Upregulated threshold 1 cluster %d.txt',i_cluster))
  DEGs <- DEGs_collection_filtered[[i_cluster]]$X[DEGs_collection_filtered[[i_cluster]]$avg_log2FC<=-1]
  writeLines(DEGs,sprintf('Downregulated threshold 1 cluster %d.txt',i_cluster))
  
  DEGs <- DEGs_collection_filtered[[i_cluster]]$X[DEGs_collection_filtered[[i_cluster]]$avg_log2FC>=2]
  writeLines(DEGs,sprintf('Upregulated threshold 2 cluster %d.txt',i_cluster))
  DEGs <- DEGs_collection_filtered[[i_cluster]]$X[DEGs_collection_filtered[[i_cluster]]$avg_log2FC<=-2]
  writeLines(DEGs,sprintf('Downregulated threshold 2 cluster %d.txt',i_cluster))
}

consensus_up <- DEGs_collection_filtered[[1]]$X[DEGs_collection_filtered[[1]]$avg_log2FC>0]
consensus_down <- DEGs_collection_filtered[[1]]$X[DEGs_collection_filtered[[1]]$avg_log2FC<0]
for(i_cluster in 2:5)
{
  consensus_up <- intersect(consensus_up,
                            DEGs_collection_filtered[[i_cluster]]$X[DEGs_collection_filtered[[i_cluster]]$avg_log2FC>0])
  consensus_down <- intersect(consensus_down,
                            DEGs_collection_filtered[[i_cluster]]$X[DEGs_collection_filtered[[i_cluster]]$avg_log2FC<0])
}
writeLines(consensus_up,'Five cluster consensus up.txt')
writeLines(consensus_down,'Five cluster consensus down.txt')
# EMT related genes
